...
def train(model, loader, filename, criterion, scheduler, epochs=1, save=True):
    ...
    for epoch in tqdm(range(1, epochs+1), desc="Training"):
        ...
        early_stopper = EarlyStopping(best_model_path=f'{filename}_model.pth')
        early_stopper(avg_loss, model, save)
...
if __name__ == "__main__":
    ...
    filename = "SPY_10k_FORMATTED_TEXT_TRAIN"
    load_model = False
    save = True
    next_day = True
    epochs = 10000
    ...
    train_dataset = DaySequenceDataset(X_train, y_train, seq_len=10)
    test_dataset  = DaySequenceDataset(X_test,  y_test, seq_len=10)
    ...
    loss_list = train(price_model, train_loader, filename, criterion, scheduler, epochs=epochs, save=save)
    ...
    plots.plot_preds_vs_target(preds, y_true, vars, filename)
    plots.plot_loss(loss_list, filename)